...
def has_write_permission(db: Session, user_id: UUID, folder_id: UUID = None, document_id: UUID = None) -> bool:
    """
    Check if user has EDITOR-level (write) permission for a folder or document.
    VIEWER permission is read-only and will return False.
    ...
    filters = [Permission.user_id == user_id, Permission.access_level == "EDITOR"]
    if folder_id:
        filters.append(Permission.folder_id == folder_id)
    else:
        filters.append(Permission.document_id == document_id)
